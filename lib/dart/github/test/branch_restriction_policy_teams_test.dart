import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for BranchRestrictionPolicyTeams
void main() {
    var instance = new BranchRestrictionPolicyTeams();

  group('test BranchRestrictionPolicyTeams', () {
    // String description (default value: null)
    test('to test the property `description`', () async {
      // TODO
    });

    // String htmlUrl (default value: null)
    test('to test the property `htmlUrl`', () async {
      // TODO
    });

    // int id (default value: null)
    test('to test the property `id`', () async {
      // TODO
    });

    // String membersUrl (default value: null)
    test('to test the property `membersUrl`', () async {
      // TODO
    });

    // String name (default value: null)
    test('to test the property `name`', () async {
      // TODO
    });

    // String nodeId (default value: null)
    test('to test the property `nodeId`', () async {
      // TODO
    });

    // String parent (default value: null)
    test('to test the property `parent`', () async {
      // TODO
    });

    // String permission (default value: null)
    test('to test the property `permission`', () async {
      // TODO
    });

    // String privacy (default value: null)
    test('to test the property `privacy`', () async {
      // TODO
    });

    // String repositoriesUrl (default value: null)
    test('to test the property `repositoriesUrl`', () async {
      // TODO
    });

    // String slug (default value: null)
    test('to test the property `slug`', () async {
      // TODO
    });

    // String url (default value: null)
    test('to test the property `url`', () async {
      // TODO
    });


  });

}
