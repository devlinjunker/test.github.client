...
class PublicUser {
  ...
  @override
  String toString() {
    return 'PublicUser[avatarUrl=$avatarUrl, bio=$bio, blog=$blog, collaborators=$collaborators, company=$company, createdAt=$createdAt, diskUsage=$diskUsage, email=$email, eventsUrl=$eventsUrl, followers=$followers, followersUrl=$followersUrl, following=$following, followingUrl=$followingUrl, gistsUrl=$gistsUrl, gravatarId=$gravatarId, hireable=$hireable, htmlUrl=$htmlUrl, id=$id, location=$location, login=$login, name=$name, nodeId=$nodeId, organizationsUrl=$organizationsUrl, ownedPrivateRepos=$ownedPrivateRepos, plan=$plan, privateGists=$privateGists, publicGists=$publicGists, publicRepos=$publicRepos, receivedEventsUrl=$receivedEventsUrl, reposUrl=$reposUrl, siteAdmin=$siteAdmin, starredUrl=$starredUrl, subscriptionsUrl=$subscriptionsUrl, suspendedAt=$suspendedAt, totalPrivateRepos=$totalPrivateRepos, twitterUsername=$twitterUsername, type=$type, updatedAt=$updatedAt, url=$url, ]';
  }
  ...
  static List<PublicUser> listFromJson(List<dynamic> json) {
    return json == null ? List<PublicUser>() : json.map((value) => PublicUser.fromJson(value)).toList();
  }

  static Map<String, PublicUser> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, PublicUser>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = PublicUser.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of PublicUser-objects as value to a dart map
  static Map<String, List<PublicUser>> mapListFromJson(Map<String, dynamic> json) {
    var map = Map<String, List<PublicUser>>();
     if (json != null && json.isNotEmpty) {
       json.forEach((String key, dynamic value) {
         map[key] = PublicUser.listFromJson(value);
       });
     }
     return map;
  }
